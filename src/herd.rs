...
/// [`get`][Herd::get]. Once the returned [`BumpSync`] proxies get dropped, they return back in here.
...
impl Herd {
    ...
    /// [`BumpSync`] is a proxy around [`Bump`], allowing to allocate objects with lifetime of the
    /// [`Herd`] (therefore, the allocated objects can live longer than the [`BumpSync`] itself).
    ///
    /// # Performance note
    ///
...
impl<'h> BumpSync<'h> {
    ...
    pub fn try_alloc_layout(&self, layout: Layout) -> Result<NonNull<u8>, AllocError> {
        self.arena.as_ref().try_alloc_layout(layout)
    }

    pub fn dealloc(&self, ptr: NonNull<u8>, layout: Layout) {
        unsafe { self.arena.as_ref().dealloc(ptr, layout) }
    }

    pub fn shrink(&self, ptr: NonNull<u8>, old_layout: Layout, new_layout: Layout) -> Result<NonNull<u8>, AllocError> {
        unsafe { self.arena.as_ref().shrink(ptr, old_layout, new_layout) }
    }

    pub fn grow(&self, ptr: NonNull<u8>, old_layout: Layout, new_layout: Layout) -> Result<NonNull<u8>, AllocError> {
        unsafe { self.arena.as_ref().grow(ptr, old_layout, new_layout) }
    }
}
...
impl<'h> BumpSync<'h> {
     ...
     * the BumpSync can be dropped and we move the Box, the Bump inside stays at the same place.
    ...
    // Note: This *can't* return `&'h Bump`. That way one could keep a reference, drop the BumpSync
...
unsafe impl<'a> Allocator for &'a BumpSync<'_> {
    #[inline]
    ...
    unsafe fn shrink(
        ...
        BumpSync::shrink(self, ptr, old_layout, new_layout)
            .map(|p| unsafe {
    ...
    unsafe fn grow(
        ...
        BumpSync::grow(self, ptr, old_layout, new_layout)
            .map(|p| unsafe {
                NonNull::new_unchecked(ptr::slice_from_raw_parts_mut(p.as_ptr(), new_layout.size()))
            })
            .map_err(|_| AllocError)
